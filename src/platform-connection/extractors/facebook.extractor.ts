...
export class FacebookExtractor implements IExtractor {
  ...
  async process(
    ...
    const {
      ...
      retrievedFrom,
      retrievedUntil,
    } = await this.facebookApiService.getFeed(
      credential.authToken,
      credential.profileId,
      { retrieveFrom: from, retrieveUntil: until },
    );

    for (const i in feedContent) {
      ...
      switch (item.status_type) {
        case 'shared_story':
          await this.ruleService.trigger('user.published.share', {
            userid: userid,
            user,
            platform: 'facebook',
        ...
        case 'added_video':
          await this.ruleService.trigger('user.published.post.video', {
            userid: userid,
            user,
            platform: 'facebook',
            message: item.message,
        ...
        case 'added_photos':
          await this.ruleService.trigger('user.published.post.photo', {
            userid: userid,
            user,
            platform: 'facebook',
            message: item.message,
